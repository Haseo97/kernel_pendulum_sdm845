		...
		*tmp = *mpnt;
		...
		if (file) {
			...
			if (tmp->vm_flags & VM_DENYWRITE)
				refcount_dec(&inode->i_writecount);
			i_mmap_lock_write(mapping);
			if (tmp->vm_flags & VM_SHARED)
				refcount_inc(&mapping->i_mmap_writable);
...
static void check_mm(struct mm_struct *mm)
{
	...
	for (i = 0; i < NR_MM_COUNTERS; i++) {
		long x = refcount_read(&mm->rss_stat.count[i]);
...
#endif
	retval = -EAGAIN;
	if (refcount_read(&p->real_cred->user->processes) >=
	...
	*/
	...
	if (likely(p->pid)) {
		...
		if (thread_group_leader(p)) {
			...
			refcount_inc(&current->signal->live);
...
bad_fork_cleanup_count:
	refcount_dec(&p->cred->user->processes);
	exit_creds(p);
bad_fork_free:
	p->state = TASK_DEAD;
	put_task_stack(p);
